...
def check_run(run_dir: Path) -> dict[str, Any]:
    acceptance_path = run_dir / "artifacts" / "acceptance-result.json"
    if acceptance_path.is_file():
        try:
            task_type = str(load_json_object(acceptance_path).get("task_type") or "").lower()
        except Exception:
            task_type = ""
        if task_type and task_type != "salvage":
            return {"check": "salvage_hard_gates", "applicable": False, "passed": True, "violations": [], "reason": f"task_type is {task_type}, not salvage"}
