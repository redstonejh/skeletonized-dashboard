...
Registry definitions may provide local demo data for data-bound widgets. Demo rows, schemas, and semantic mappings come from the centralized demo data runtime in `app/static/demo-data.js` and are queried through the same local record adapter path only when no real workspace data source is resolved. Demo layout presets live separately in `app/static/demo-layout.js` so fake rows are not hardcoded into widget renderers or layout mechanics. Demo query results are runtime-only: they must not be written into widget config, saved layout state, panel child ownership, or persisted workspace snapshots. Real workspace context and explicit data-source bindings always take precedence over demo data, including honest empty/configuration states when a real source lacks required fields. See `docs/demo-simulation-system.md` for the seeded data generator, transform utilities, use-case matrix, and demo workspace presets.
