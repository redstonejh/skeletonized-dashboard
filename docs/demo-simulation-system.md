...
- `app/static/demo-data.js`: deterministic data generation, reusable data transforms, safe expression evaluation, widget demo-data selection, scenario sources, and the widget/data use-case matrix.
- `app/static/demo-layout.js`: deterministic demo workspace preset definitions, preset ordering, generated-profile helpers, and AI example layout metadata.
- `app/static/data-simulation-runtime.js`: compatibility bridge for older callers that still expect `window.dashboardDemoDataRuntime.workspacePresets()`.
- `app/static/app.js`: applies demo layouts through the existing source-agnostic context/query adapter and exposes `window.dashboardDemoWorkspaceRuntime`.
...
The data runtime exposes `window.dashboardDemoDataRuntime.useCaseMatrix()`, and `window.dashboardDemoWorkspaceRuntime.useCaseMatrix()` remains as a workspace-facing convenience wrapper.
...
`window.dashboardDemoLayoutRuntime.presets()` owns the preset definitions. `window.dashboardDemoWorkspaceRuntime.applyPreset(presetId, options)` applies one of those definitions by creating seeded data sources, root workspace context, widgets, panels, and panel-local widgets.
...
Preset layout objects are normal registry-backed widgets and panels. Data source rows are generated by `demo-data.js` and consumed by `demo-layout.js` definitions only through the normal layout application path. Transient widget query results are not written into widget config, panel child ownership, or saved layout snapshots.
