...
- Fix notes: Added `app/static/widget-registry.js` with registry-backed definitions for stat, timeframe, search, table, chart/graph, data filter, and calendar widgets. `app/static/app.js` now creates widgets through registry definitions, hydrates existing placeholder widgets into runtime metadata, persists `runtimeType` and config, lets widgets resolve current workspace context, asks definitions for source-neutral `ContextQuery` objects, renders normalized query results, and shows a generic unsupported-widget state for missing definitions. Smart-delete now compares widget config against registry defaults so newly created blank widgets still delete without confirmation.
...
- Observed: Panels visually represented generic containers, but dragging a widget over an open panel had no direct containment path. Earlier containment used a delayed receptive hover concept that felt like a separate mode instead of seamless panel-local dragging.
- Expected: Open panels should behave like scoped internal workspaces. A widget dragged into an open panel body should immediately transition its ghost and placement preview into the panel-local grid, while dragging over the header/collapse region or a collapsed panel continues to use normal workspace collision.
...
- Fix notes: Added scoped panel internal widget grids, panel child serialization, restore, undo/redo, save/load, and reset cleanup. Dashboard collision, FLIP snapshots, reflow item queries, and local/global occupancy helpers now exclude panel child widgets from top-level workspace calculations. Widget drag now switches preview scope directly when the pointer enters an open panel body, uses a panel-local placeholder and sibling reflow while inside, returns cleanly to workspace preview when leaving, and commits the widget as panel child state on release. Child widgets reuse normal widget controls and can be moved inside the panel grid.
- Validation: Added `test_widget_drags_directly_into_open_panel_and_round_trips`, `test_deleting_panel_extracts_child_widgets_and_undo_restores_containment`, and `test_widget_hover_over_collapsed_panel_does_not_absorb`. Targeted containment, save/load, undo, drag preview cleanup, widget drag, panel collision, group drag, and group resize slices passed. Manual browser inspection verified direct panel-local preview, containment commit, child widget controls, deep background rendering, and no persistent top-level duplicate. Full `.venv\Scripts\python.exe -m pytest -q` passed with 97 tests and 1 responsive/mobile test skipped by the desktop-iteration gate.

### BUG-110: Panel Header Could Block Intentional Containment When No Outside Room Existed

- Status: Verified
- Area: Dashboard grid / panel containment / widget drag lifecycle
- Severity: Medium
- Environment: Dashboard workspace, desktop Chromium, dragging a widget directly above an open panel
- Observed: Header and chevron regions were always treated as normal workspace collision, so a widget directly above an open panel could not naturally enter the panel when there was no clear gap before the panel body.
- Expected: The panel body remains the primary direct entry zone, but a slow downward drag into an open panel header should be accepted as containment intent. Fast header pass-through and collapsed panels should continue using normal workspace collision.
- Suspected cause: Panel entry detection only distinguished body entry from non-body collision and lacked a gesture-quality layer for the header.
- Fix notes: Added a panel entry candidate resolver that separates body and header zones. Header entry now requires slow/downward intent with only a tiny dwell-quality check, triggers a short one-shot `panel-header-entry-accept` pulse, and then switches into the existing panel-local grid preview path. Fast header passes continue through workspace collision.
- Validation: Added `test_slow_header_drag_can_enter_open_panel_when_no_outside_room` and `test_fast_header_pass_through_keeps_workspace_collision`. Targeted containment, sparse drop, global occupancy, drag ghost, menu suppression, group drag, and group resize slices passed. Full `.venv\Scripts\python.exe -m pytest -q` passed with 100 tests and 1 responsive/mobile test skipped by the desktop-iteration gate.
...
### BUG-074: Search Bar Input Collided With Widget Settings Control

- Status: Verified
- Area: Widgets / layout / theme
- Severity: Medium
- Environment: Dashboard workspace, Chromium desktop, default frosted-light and deep-slate backgrounds
- Observed: The Search Bar widget reused the generic range-search field inside a minimum-size widget, but the generic widget padding won over the Search Bar's reserved settings lane. The input overlapped the right-side settings button area, sat slightly high, used small label typography, and the filled state allowed the floating label to collide with typed text.
- Expected: Search Bar widgets should use a balanced flex row with a compact embedded input, readable search label/text, integrated search icon, vertical centering, and fixed breathing room before the widget settings button at default and minimum widths.
- Suspected cause: `.widget-layout > .stat-card` padding specificity overrode the Search Bar padding, while the shared `.range-search` label behavior was tuned for larger floating-label controls rather than a compact widget-contained search field.
- Fix notes: Scoped the Search Bar widget through the shared `range-search` component path with a stronger widget-layout selector, explicit flex centering, `min-width: 0` content/control sizing, a 66px settings-control lane, 36px compact input height, larger label/input typography, refined search icon sizing, and placeholder-like label hiding once the field is focused or filled.
- Validation: Updated `test_search_bar_widget_uses_normal_widget_creation_and_controls` with bounding-box assertions for input/settings separation, vertical centering, compact height, min-width behavior, readable typography, and icon presence. Targeted desktop test passed; manual inspection screenshots were captured in `test-results/manual-search-widget/`.

### BUG-075: Added Objects Spawned From Fixed Defaults Instead Of The Visible Workspace Region

- Status: Verified
- Area: Dashboard grid / insertion / dividers
- Severity: Medium
- Environment: Dashboard workspace, Chromium desktop, scrolled divider regions
- Observed: Adding a widget, panel, or divider-supported object used fixed/default append behavior. New objects could appear away from the divider region the user was currently viewing, and widgets relied on auto placement rather than an explicit committed grid target.
- Expected: Add actions should choose the divider-defined workspace region with the most visible available screen space, place the object below that divider, use the first ordered valid slot, fall back to the next available slot when occupied, and preserve normal undo/save/load behavior.
- Suspected cause: Panel/divider creation used a panel-only last-item target, while widget creation appended without a shared region-aware target. Existing insertion did not score current viewport visibility or current live divider geometry.
